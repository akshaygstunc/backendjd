...
export const validateVerifyUser = (req, res, next) => {
  const { error } = verifyUser.validate(req.query);
...
const paymentDetails = Joi.object({
  ...
  payment_status: Joi.string()
    ...
    .messages({
      "any.empty": "payment status cannot be empty",
      "any.only":
  ...
  gst_number: Joi.string().required().messages({
    "string.empty": "GST number is not allowed to be empty",
    "any.required": "GST number is required",
  }),
});

export const validatePaymentDetails = (req, res, next) => {
  const { error } = paymentDetails.validate(req.body);
